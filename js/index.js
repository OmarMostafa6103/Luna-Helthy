...
function searchProduct() {
    ...
    allProducts.forEach((product) => {
        ...
        if (productName.includes(searchQuery)) {
            ...
            // إعداد زر الإضافة للعربة
            const addCartBtn = newProductCard.querySelector('.add-cart-btn');
            if (addCartBtn) {
...
// JavaScript للتحكم في النافذة المنبثقة
...
// فتح النافذة المنبثقة عند النقر على زر إتمام الشراء
checkoutBtn.addEventListener('click', function () {
    phoneModal.style.display = 'block';
    overlay.style.display = 'block';
    formContent.style.display = 'block'; // إظهار حقول الإدخال والأزرار
    cancelMessage.style.display = 'none'; // إخفاء رسالة الإلغاء
});

// إغلاق النافذة المنبثقة عند النقر على زر الإلغاء
cancelPhone.addEventListener('click', function () {
    phoneInput.value = ''; // مسح قيمة حقل إدخال رقم الهاتف
    formContent.style.display = 'none'; // إخفاء حقول الإدخال والأزرار
    cancelMessage.style.display = 'block'; // إظهار رسالة الإلغاء

    // إغلاق النافذة بعد 2 ثانية
    setTimeout(function () {
        phoneModal.style.display = 'none';
        overlay.style.display = 'none';
    }, 1000); // 2000 ميلي ثانية = 2 ثانية
});

// التحقق من صحة رقم الهاتف عند النقر على زر الإرسال
submitPhone.addEventListener('click', function () {
    const phoneNumber = phoneInput.value;

    // تحقق من صحة رقم الهاتف
    const phoneRegex = /^(011|010|015|012)\d{8}$/;
    if (!phoneRegex.test(phoneNumber)) {
        errorMessage.style.display = 'block'; // إظهار رسالة الخطأ
        phoneInput.focus(); // إعادة التركيز على حقل إدخال رقم الهاتف
        return;
    }

    // إخفاء رسالة الخطأ إذا كان الرقم صحيحًا
    ...
    // تفاصيل المنتجات (يمكن استبدالها ببيانات حقيقية)
    ...
    const email = "lunahealthy2001@gmail.com"; // البريد الإلكتروني للمستلم
    const subject = "تفاصيل شراء المنتجات"; // موضوع البريد الإلكتروني
    const body = encodeURIComponent(message); // ترميز محتوى البريد الإلكتروني
